<!-- FOOTER PHP FRAGMENT -->
<footer class="page-footer text-center font-small wow fadeIn">
    <div>
        <a class="btn btn-outline-white"
           href="../pcdus_customresources/docs/Paul_Curtis_Resume_2020.docx" target="_blank"
           role="button">Download CV / Resume<i class="fas fa-download ml-2"></i>
        </a>
    </div>
    <div class="pb-4">
        <a href="https://linkedin.com/in/paulcphilly/" target="_blank"> <img
                    alt="LI" src="../pcdus_customresources/img/pcdus-li-icon-sm.png" height="64px"
                    width="64px"/>
        </a> <a href="https://github.com/pcurtis5688" target="_blank"> <img
                    alt="Git" src="../pcdus_customresources/img/GitHub-Mark-64px.png">";
        </a>
    </div>
    <div class="footer-copyright py-3">Paul Curtis || pcurtis5688@gmail.com
        || 412-716-0747 || 484-442-0052
    </div>
</footer>